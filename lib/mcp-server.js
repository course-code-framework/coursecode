        ...
        try {
            ...
            switch (name) {
                ...
                case 'coursecode_state':
                    ...
                    // Read error log from preview server (same errors user sees in debug panel)
                    try {
                ...
                case 'coursecode_lint':
                    ...
                    // Merge runtime errors from the preview server's error log.
                    // Read from the server HTTP API (not the headless browser) so we see
                    // the same errors the user sees in the debug panel Errors tab.
                    {
                        try {
                            const resp = await fetch(`http://localhost:${port}/__lms/errors`);
                            if (resp.ok) {
                                const data = await resp.json();
                                const serverErrors = data.errors || [];
                                const serverWarnings = data.warnings || [];
                                result.runtimeLintIncluded = true;
                                if (serverWarnings.length > 0) {
                                    result.warnings = (result.warnings || []).concat(
                                        serverWarnings.map(e => ({
                                            severity: 'warning',
                                            type: e.type,
                                            message: e.message,
                                            hint: e.hint,
                            ...
                            // Preview server may not be running — non-fatal, build lint still valid
                        }
                    }
                    break;
